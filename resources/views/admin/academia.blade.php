...
<body>
    ...
    @if(isset($treinamento))
        ...
        <form action="{{ route('admin.academia.update', $treinamento->id) }}" method="POST">
            ...
            <!-- Campo "Link" removido, pois o acesso será feito via rota interna -->
            <label>Categoria:
        ...
        <tbody>
            @forelse($treinamentos as $t)
                <tr>
                    ...
                    <td>{{ $t->descricao }}</td>
                    <td>
                        <!-- Link interno para acessar o treinamento -->
                        <a href="{{ route('treinamentos.show', $t->id) }}" target="_blank">Acessar Treinamento</a>
                        <!-- Botão de Editar -->
